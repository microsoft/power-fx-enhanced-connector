...
Tabular Connectors allow Power Platform to connect to tabular data in datasources. Whereas action connectors can be described as a fixed list of method calls in OpenApi.Net ("swagger"), tabular connectors need to describe a dynamic set of tables, their schemas and capabilities. 

This describes the Tabular Connector Protocol. This protocol is a RESTful protocol primarily based on OData and augmented with metadata to be self-describing. 
...
Power Fx expressions provide an easy way to access tabular connectors.  `Delegation` refers to translating a Power Fx function into an efficieint server-side operation. The Power Fx engine will look at the connector capabilities and produce a query. 
...
Tabular Connectors use optimistic concurrency via an etag pattern. 
